    ...
    <span style="display:none;">{{selected}} {{truc}}</span>
          <v-list two-line>
            <template  v-for="(item, index) in points">
              <v-list-tile :key="'tile-'+index" avatar>
                ...
                <v-list-tile-content :key="'content-'+item.id">
                  ...
                  <v-list-tile-sub-title v-for="description in item.description" v-if="description.langCode=='fr'" :key="'subtile-'+description.descriptiontext">
